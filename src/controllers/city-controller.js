...
const createMutlipleCities = async (req, res) => {
  try {
    await cityService.createMultipleCities(req.body.cities);
    return res.status(200).json({
      success: true,
      message: "Succesfully created multiple cities",
    });
  } catch (error) {
    console.log(error);
    return res.status(500).json({
